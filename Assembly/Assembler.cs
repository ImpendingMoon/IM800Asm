...
internal partial class Assembler
{
	...
	///     Sanity check
	/// </summary>
	private int _bytesEmittedForCurrentStatement;
	...
	public Assembler(List<Statement> statements)
	{
		...
		_evaluator = new ExpressionEvaluator(GetSymbolValue);
		_data = [];
	}
		...
		foreach (Statement st in _statements)
		{
			...
			if (st is LabelStatement ls)
			{
				...
				else
				{
					result.AddError(
						ls.SourceLocation,
						Constants.ErrorCode.RedefinedSymbol,
						$"cannot redefine symbol \"{ls.Text}\""
					);
				}
			}
			...
			else if (st is InstructionStatement inst)
			{
				...
				else
				{
					result.AddError(
						...
						$"invalid operands for instruction {inst.Instruction}"
					);
				}
	...
	private bool TryDefineSymbol(string symbol, long value)
	{
		...
		_symbolTable[qualifiedName] = new Symbol(Constants.SymbolType.Label, qualifiedName, value);
		return true;
	}
	...
	{
		string qualifiedName = GetQualifiedName(symbol);
		_symbolTable[qualifiedName] = new Symbol(Constants.SymbolType.EQU, qualifiedName, value);
	}
	...
	private long? GetSymbolValue(string symbol)
	{
		...
		if (!_symbolTable.TryGetValue(qualifiedName, out Symbol? value))
		{
			return null;
		}
