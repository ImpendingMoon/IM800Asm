...
internal static class InstructionTable
{
	private static readonly Dictionary<Constants.Instruction, List<Entry>> _instructionTable = new()
	{
		...
		[
			new Entry(
				...
				0b10000,
				0,
				true // LEA adds a 16-bit value to a 32-bit value
			),
			new Entry(
				[
					new OperandSlot(Constants.AcceptedOperandType.WideRegister),
		...
		[Constants.Instruction.BIT] = MakeStandardRRMEntries(0b10001, true),
		[Constants.Instruction.SET] = MakeStandardRRMEntries(0b10010, true),
		[Constants.Instruction.RES] = MakeStandardRRMEntries(0b10011, true),
		[Constants.Instruction.RLC] = MakeStandardRRMEntries(0b10100, true),
		[Constants.Instruction.RRC] = MakeStandardRRMEntries(0b10101, true),
		[Constants.Instruction.RL] = MakeStandardRRMEntries(0b10110, true),
		[Constants.Instruction.RR] = MakeStandardRRMEntries(0b10111, true),
		[Constants.Instruction.SLA] = MakeStandardRRMEntries(0b11000, true),
		[Constants.Instruction.SRA] = MakeStandardRRMEntries(0b11001, true),
		[Constants.Instruction.SRL] = MakeStandardRRMEntries(0b11010, true),
		// UR/UM
	...
	public static bool TryResolveInstruction(InstructionStatement statement, out Entry? resolvedEntry)
	{
		...
		if (!_instructionTable.TryGetValue(statement.Instruction, out List<Entry>? entries))
		{
	...
	{
		public List<OperandSlot> Slots { get; set; } = slots;
		public Constants.InstructionFormat InstructionFormat { get; set; } = instructionFormat;
		public List<Constants.Size> AllowedSizes { get; set; } = allowedSizes;
		public Constants.Size DefaultSize { get; set; } = defaultSize;
		public int Opcode { get; set; } = opcode;
		public int Function { get; set; } = function;
		...
		/// </summary>
		public bool AllowMixedSizes { get; set; } = allowMixedSizes;
